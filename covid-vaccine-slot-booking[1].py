...
def main():
    ...
    #filename='vaccine-booking-details-7892125302.json'
    mobile = None
    ...
    try:
        ...
        if args.token:
            ...
            while token is None:
                if otp_pref=="n":
                    try:
                        token, captcha = generate_token_OTP(mobile, base_request_header, captcha_automation, captcha_automation_api_key )
                    except Exception as e:
                            print(str(e))
                            print('OTP Retrying in 5 seconds')
                            time.sleep(5)
                elif otp_pref=="y":
                    token, captcha = generate_token_OTP_manual(mobile, base_request_header, captcha_automation, captcha_automation_api_key)
        ...
        while token_valid:
            ...
            try:
                ...
                while token is None:
                    if otp_pref=="n":
                        try:
                           token, captcha = generate_token_OTP(mobile, base_request_header, info.captcha_automation,info.captcha_automation_api_key )
                        except Exception as e:
                            print(str(e))
                            print('OTP Retrying in 5 seconds')
                            time.sleep(5)
