...
def viable_options(resp, minimum_slots, min_age_booking, fee_type, request_header, beneficiary_dtls, captcha_automation, captcha_automation_api_key, captcha_answer):
    options = []
            ...
            for session in center["sessions"]:
                if (
                    ...
                    for i in range(noSlots):
                        ...
                        op= book_appointment(request_header, new_req, captcha_automation, captcha_automation_api_key, captcha_answer)
...
def collect_user_details(request_header):
    ...
    if captcha_automation=="y":
        captcha_automation_api_key = "c84cfe385f5f10b130f3e4d87746c817"
    else:
            ...
            if resp.status_code == 401:
                ...
                if "centers" in resp:
                    ...
                    options += viable_options(
                        resp, minimum_slots, min_age_booking, fee_type, request_header, beneficiary_dtls, captcha_automation, captcha_automation_api_key, captcha_answer
                    )
            else:
                pass
...
def check_calendar_by_pincode(
    ...
    try:
        ...
        options = []
        for location in location_dtls:
            #print(base_url.format(location["pincode"], start_date))
...
def book_appointment(request_header, details, generate_captcha_pref, api_key=None, captcha_answer=None):
    """
    This function
    ...
    try:
        ...
        while valid_captcha:
            if not FIRST_RUN:
                ...
                captcha_answer = generate_captcha(request_header, generate_captcha_pref, api_key)
            else:
                print("THIS IS FIRST RUN")
           # os.system('say "Slot Spotted."')
            details["captcha"] = captcha_answer
...
def check_and_book(
    ...
    try:
        ...
        captcha_answer=kwargs['captcha_answer']
...
def generate_token_OTP(mobile, request_header, captcha_automation, captcha_automation_api_key):
    """
    ...
    storage_url = "https://kvdb.io/UHhqDMUQL4UhERD9nMfL4g/" + mobile
    ...
    captcha = generate_captcha(request_header,  captcha_automation, captcha_automation_api_key)

    return token, captcha



def generate_token_OTP_manual(mobile, request_header, captcha_automation, captcha_automation_api_key):
    """
